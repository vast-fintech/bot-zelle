...
def operate():
    ...
    def cancelar():
        accion_votc = driver.find_element_by_xpath('//*[@id="root"]/div/main/div[1]/div[2]/div/div[3]/table/tbody/tr['+str(last+1)+']/td/div/div/div/div/div[2]/div[2]/div/div/div/div')
        ...
        if claims > 0:
            ...
            for row in transact_dict:
                ...
                if row['Fecha']==datetime.today().date() and fuzz.ratio(str(row['Nombre']),str(remitente))>=68 and discrepancy < cascade:
                    ...
                    break
                elif fuzz.partial_ratio(str(row['Nombre']),str(remitente))>=62 and discrepancy < cascade:
                    print('Transacción encontrada, posteada el',str(row['Fecha']))
                    print(frame())
                    gapi(3).sheets(frame(),'F',index)
                    confirmar()
