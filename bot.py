    ...
    while TRUE:
        ...
        if claims > 0:
            ...
            for row in transact_dict:
                ...
                if row['Fecha']==datetime.today().date() and fuzz.ratio(str(row['Nombre']),str(remitente))>=67 and discrepancy < cascade:
                    print('Transacción encontrada, posteada hoy')
                    print(frame())
                    gapi(3).sheets(frame(),'F',index)
                    confirmar()
                    break
                elif fuzz.ratio(str(row['Nombre']),str(remitente))>=67 and discrepancy < cascade:
